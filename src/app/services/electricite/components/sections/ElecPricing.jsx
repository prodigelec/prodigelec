...
export default function ElecPricing() {
  ...
  return (
    <section className="py-16">
      <h2 className="text-3xl font-bold text-white mb-12 text-center">Mes Tarifs <span className="text-accent">Électricité</span></h2>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {prices.map((plan, idx) => (
          <div
            key={idx}
            className={`rounded-3xl p-8 border ${plan.highlight ? 'border-accent bg-accent/5' : 'border-white/10 bg-white/5'} relative flex flex-col`}
          >
            {plan.highlight && (
            ...
            <div className="text-3xl font-black text-accent mb-4">{plan.price}</div>
