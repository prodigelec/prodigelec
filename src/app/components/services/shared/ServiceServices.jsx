  ...
  return (
    <div className="mb-16 md:mb-32">
      <div className="text-center mb-8 md:mb-16 px-6">
        <h2 className="text-2xl md:text-4xl lg:text-5xl font-black text-white mb-4 uppercase italic tracking-tighter">
      ...
      <div className={`grid grid-cols-1 md:grid-cols-2 ${theme === 'electricite' ? 'lg:grid-cols-3' : 'lg:grid-cols-2'} gap-4 md:gap-8 lg:gap-10 px-6`}>
        {features.map((feature, idx) => (
          ...
          >
            <div className={`absolute -top-24 -right-24 w-48 h-48 ${config.glowColor} rounded-full blur-[80px] transition-colors duration-700`} />
            ...
            <p className="text-gray-100 mb-6 md:mb-8 text-sm md:text-base lg:text-lg leading-relaxed relative z-10">
              {feature.description}
            </p>

            <div className="w-full h-px bg-linear-to-r from-white/10 to-transparent mb-6 md:mb-8 relative z-10" />

            <ul className="space-y-3 md:space-y-4 relative z-10">
              {feature.items.map((item, i) => (
                <li key={i} className="flex items-center gap-3 text-gray-100">
                  <div className={`w-5 h-5 md:w-6 md:h-6 rounded-full ${config.checkBg} flex items-center justify-center shrink-0`}>
