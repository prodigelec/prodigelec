    ...
    return (
        <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-[9999] pointer-events-none">
            <motion.div
                ...
                className="bg-[#0b1a2a]/95 backdrop-blur-2xl border-t border-white/10 rounded-t-[2rem] px-6 py-4 pb-8 shadow-2xl flex items-center justify-between w-full pointer-events-auto"
            >
                {navItems.map((item, i) => (
                    <motion.a
                        key={item.name}
