...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    openGraph: {
      ...
      images: meta.image
        ? [{ url: `https://www.prodigelec.fr${meta.image}`, alt: meta.imageAlt || meta.titre }]
        : [],
    },
  }
}
...
export default async function BlogPostPage({ params }) {
  ...
  const serviceLink = serviceLinks[meta.categorie]
  ...
  const blogPostingSchema = {
    ...
    "mainEntityOfPage": {
      "@type": "WebPage",
      "@id": `https://www.prodigelec.fr/blog/${slug}`
    }
  }
    ...
    <>
      ...
      <main className="min-h-screen bg-background text-foreground pt-20 pb-16 mt-16 md:pt-24 md:mt-16">
        <div className="max-w-3xl mx-auto px-6">
          ...
          {serviceLink && (
            <div
              className="mt-10 flex flex-col sm:flex-row items-start sm:items-center gap-3 p-5 rounded-2xl"
            ...
            >
              ...
              >
                {serviceLink.label} <ArrowRight size={13} />
              </Link>
            </div>
          )}
