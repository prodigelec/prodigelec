...
import { Phone, Globe, Gift, ShieldCheck, BadgeCheck, Sparkles } from "lucide-react";
...
export default function FlyerContent() {
  return (
    <SocialFlyerShell
      editorTitle="Éditeur Flyer Social — Vacances"
...
function SocialLayout() {
  const { accent: a, dateFormattedUpper, phoneNumber } = useSocialFlyer();

  return (
    <div className="w-full h-full flex flex-col">
      {/* HEADER (180px) avec image de fond */}
      <div
        className="relative px-12 pt-8 pb-5 flex items-center justify-between bg-[#0b1a2a] border-b border-amber-500/20 shrink-0 overflow-hidden"
        style={{ height: "180px" }}
      >
        {/* Background image */}
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-14 py-6 bg-linear-to-b from-[#0b1a2a] via-[#0f2540] to-[#0b1a2a] relative overflow-hidden">
        ...
        <p className={`relative text-slate-200 text-lg font-bold tracking-wide mt-3 text-center`}>
          <span className={a.text400}>Alarme connectée</span> &amp; <span className={a.text400}>vidéosurveillance</span> — sécurité totale.
        </p>
        ...
        >
          ...
          <div className="absolute bottom-4 left-5 right-5 flex items-center justify-between">
            ...
            <div className="inline-flex items-center gap-2 bg-amber-500/15 border border-amber-500/40 backdrop-blur-md rounded-lg px-3 py-1.5">
              <Sparkles className={`w-4 h-4 ${a.text400}`} />
      ...
      {/* OFFER BAND (180px) */}
      <div
        className="bg-[#0b1a2a] px-12 py-5 border-t border-amber-500/30 shrink-0 relative"
        style={{ height: "180px" }}
      >
        {/* Badge "Offert" */}
        ...
        <div className="flex items-center gap-6 h-full">
          ...
          >
            <Gift className={`w-10 h-10 ${a.text500}`} />
          </div>
          ...
          <div className="bg-linear-to-r from-amber-500 to-[#ffd60a] text-slate-950 font-black rounded-xl px-5 py-3 text-center shadow-lg shadow-amber-500/25 shrink-0">
      ...
      {/* FOOTER (220px) */}
      <div
        className="bg-[#0b1a2a] px-12 py-5 border-t border-amber-500/20 shrink-0 flex flex-col justify-between"
        style={{ height: "220px" }}
      >
        ...
        <div className="flex items-center justify-center gap-2 text-[13px] font-black uppercase tracking-[0.18em] text-white font-sora">
          <span>PRODIGELEC</span>
          <span className={a.text500}>•</span>
          <span>BROUÉ <span className="text-[#ffd60a]">(28)</span></span>
          <span className={a.text500}>•</span>
          <span className="text-slate-300">SIRET {contact.siret}</span>
        </div>
