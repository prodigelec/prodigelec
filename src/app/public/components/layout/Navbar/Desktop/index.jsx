...
export default function Navbar() {
    ...
    { name: "Accueil", href: "/public" },
    {
      ...
      dropdown: [
        ...
        },
        {
          ...
          icon: <FcFlashOn className="w-5 h-5" />,
        },
      ],
    },
    {
      name: "Réalisations",
      href: isHomePage ? "#realisations" : "/public#realisations",
    },
  ...
  useMotionValueEvent(scrollY, "change", (latest) => {
    ...
    if (isHomePage) {
      const sections = navItems.map((item) =>
        item.href.replace("/#", "").replace("#", "").replace("/", ""),
      );
  ...
  return (
    ...
    >
      <div
        className={`max-w-7xl mx-auto px-6 flex items-center justify-between transition-all duration-500 ${isScrolled ? "bg-black/20 rounded-full border border-white/10 px-8 mx-4 backdrop-blur-md shadow-lg" : ""}`}
      >
        {/* Logo & Brand */}
        <Logo
          className={`${isScrolled ? "scale-90" : "scale-100"} transition-transform duration-500`}
        />
          ...
          {navItems.map((item, i) => (
            <div key={item.name} className="relative group">
              <motion.a
                ...
                href={item.href}
                className={`flex items-center gap-1 text-sm font-medium transition-all duration-300 relative px-3 py-2 rounded-lg bg-black/30 backdrop-blur-sm hover:bg-black/40 ${
                  isItemActive(item)
              ...
              >
                ...
                <span
                  className={`absolute bottom-1 left-3 right-3 h-0.5 bg-primary transition-transform duration-300 ${
                    isItemActive(item)
                      ? "scale-x-100"
