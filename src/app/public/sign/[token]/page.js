...
import { Shield, FileCheck, PenTool, Download, CheckCircle2 } from 'lucide-react';
...
export default function PublicSignaturePage() {
    ...
    const fetchDocument = async () => {
        try {
            const response = await axios.get(`/api/public/signature/document?token=${token}`);
    ...
    const handleSign = async () => {
        ...
        try {
            const response = await axios.post(`/api/public/signature/signature/create?token=${token}`, {
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4">
                <div className="bg-white p-8 rounded-2xl shadow-sm text-center max-w-md w-full">
                    ...
                    <p className="text-slate-600">Ce lien de signature n'est plus valide ou a expiré.</p>
                </div>
            </div>
        );
    }
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4">
                <div className="bg-white p-10 rounded-3xl shadow-xl text-center max-w-md w-full animate-in zoom-in-95 duration-300">
                    ...
                    <p className="text-slate-600 mb-8">Le devis <strong>{document.quote_number}</strong> a été signé avec succès.</p>
                    <div className="flex flex-col gap-3">
                        ...
                        <p className="text-xs text-slate-400 mt-4">Une copie de confirmation vous a été envoyée par email.</p>
                    </div>
                </div>
            </div>
        );
            ...
            <header className="bg-white border-b border-slate-200 sticky top-0 z-30">
                <div className="max-w-4xl mx-auto px-4 h-20 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="font-bold text-slate-900 tracking-tight">PRODIGELEC</span>
                    </div>
            ...
            <main className="max-w-4xl mx-auto px-4 pt-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden mb-8">
                    ...
                    <div className="p-8 border-b border-slate-100 bg-slate-50/50">
                        <div className="flex flex-col md:row items-start justify-between gap-4">
                            <div>
                                <span className="text-xs font-bold text-blue-600 uppercase tracking-wider mb-1 block">Signature de devis</span>
                                <h1 className="text-2xl font-bold text-slate-900">Devis n° {document.quote_number}</h1>
                            </div>
                            <div className="text-right">
                                <div className="text-3xl font-bold text-slate-900">{new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR' }).format(document.total_ttc)}</div>
                                <div className="text-sm text-slate-500">Total TTC à valider</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="p-8 space-y-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div>
                                <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Prestataire</h3>
                                <div className="text-sm">
                                    <p className="font-bold text-slate-900 text-base">{document.company.name}</p>
                                    <p className="text-slate-600 mt-1">{document.company.address}</p>
                                    <p className="text-slate-600">{document.company.zip_code} {document.company.city}</p>
                                    <p className="text-slate-500 mt-2">SIRET: {document.company.siret}</p>
                                </div>
                            </div>
                            <div>
                                <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Client</h3>
                                <div className="text-sm">
                                    <p className="font-bold text-slate-900 text-base">{document.customer.first_name} {document.customer.last_name}</p>
                                    {document.customer.company_name && <p className="text-slate-800 font-medium">{document.customer.company_name}</p>}
                                    <p className="text-slate-600 mt-1">{document.customer.address}</p>
                                    <p className="text-slate-600">{document.customer.zip_code} {document.customer.city}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Détail des prestations</h3>
                                    ...
                                    <tbody className="divide-y divide-slate-100">
                                        {document.items.map((item, idx) => (
                                            <tr key={idx}>
                                                ...
                                                <td className="px-6 py-4 text-right text-slate-900 font-medium">{new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR' }).format(item.total_ht)}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                        ...
                        {document.notes && (
                            <div className="bg-amber-50/50 p-6 rounded-2xl border border-amber-100/50">
                                <h3 className="text-xs font-bold text-amber-800 uppercase tracking-widest mb-2">Notes</h3>
                ...
                <div className="bg-white rounded-3xl shadow-xl border-2 border-blue-600/20 overflow-hidden">
                    <div className="p-6 bg-blue-50/50 border-b border-blue-100 flex items-center gap-3">
                        ...
                        <h2 className="text-lg font-bold text-blue-900">Signer le document</h2>
                    </div>
                    <div className="p-8">
                        <p className="text-sm text-slate-600 mb-6">
                            En signant ci-dessous, vous acceptez les conditions générales de vente et validez les prestations décrites ci-dessus.
                        </p>
                        ...
                        <div className="flex flex-col sm:row items-center justify-between gap-6">
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    <p className="text-xs font-bold text-slate-900">Signature certifiée ProdigeLeC</p>
                                    <p className="text-[10px] text-slate-500 max-w-[200px]">Votre signature sera associée à un certificat électronique infalsifiable (RSA-SHA256).</p>
                                </div>
                            </div>
                            <button
                                onClick={handleSign}
