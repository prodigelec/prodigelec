  ...
  async getSignatureDetails(signatureId) {
    try {
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(
          error.error || "Erreur lors de la récupération des détails",
        );
      }
  ...
  async downloadCertificate(signatureId) {
    try {
      const response = await fetch(
        `${this.baseURL}/${signatureId}/certificate`,
        {
          headers: {
            Authorization: `Bearer ${this.getAuthToken()}`,
          },
  ...
  validateEIDASCompliance(signature) {
    ...
    if (!validAlgorithms.includes(signature.signature_algorithm)) {
      validation.warnings.push(
        `Algorithme non standard: ${signature.signature_algorithm}`,
      );
    }
