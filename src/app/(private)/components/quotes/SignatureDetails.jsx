...
export default function SignatureDetails({ signatureId }) {
  ...
  }, [signatureId, fetchSignatureDetails]);

  const fetchSignatureDetails = useCallback(async () => {
    try {
  ...
  const downloadCertificate = async () => {
    try {
      ...
      const response = await fetch(
        `/api/signatures/${signatureId}/certificate`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      <div className="p-6 border-b border-gray-200">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-semibold text-gray-900">
            Détails de la Signature Électronique
          </h3>
          <div
            className={`flex items-center px-3 py-1 rounded-full text-sm font-medium border ${getStatusColor(signature.status)}`}
          >
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-900">
                {format(new Date(signature.expires_at), "dd MMMM yyyy", {
                  locale: fr,
                })}
              </p>
            </div>
        ...
        <div>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <p className="text-sm text-blue-800">
              Cette signature électronique est conforme au règlement eIDAS sur
              les signatures électroniques. Elle a la même valeur juridique
