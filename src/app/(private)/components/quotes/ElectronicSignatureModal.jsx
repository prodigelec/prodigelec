    ...
    try {
      ...
      if (response.ok) {
        ...
        formData.append(
          "pdf",
          pdfBlob,
          `devis-certifie-${quote?.quote_number || "prodige"}-${Date.now()}.pdf`,
        );
        ...
        throw new Error(
          error.error || "Erreur lors de la création de la signature",
        );
      }
    } catch (error) {
        ...
        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-gradient-to-r from-blue-600 to-blue-700">
          <div>
            ...
            <p className="text-blue-100 text-sm mt-1">
              Conforme eIDAS - Vérification par OTP
            </p>
          </div>
          ...
          <div className="flex items-center justify-between">
            {[
              ...
              <div key={item.step} className="flex items-center">
                ...
                <span
                  className={`ml-2 text-sm font-medium ${currentStep >= item.step ? "text-slate-800" : "text-slate-400"}`}
                >
                  {item.label}
                </span>
                {index < 2 && (
                  <div
                    className={`w-16 h-0.5 mx-4 ${currentStep > item.step ? "bg-green-500" : "bg-slate-200"}`}
                  />
                )}
              </div>
            ))}
            ...
            <div className="space-y-4">
              <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
                ...
                <p className="text-sm text-amber-700">
                  Votre signature électronique est légalement contraignante
                  conformément au règlement eIDAS. En signant, vous acceptez les
                  termes du devis.
                </p>
              </div>
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                ...
                <p className="text-sm text-blue-700">
                  ...
                  <strong>{customer?.email}</strong> pour confirmer votre
                  identité.
                </p>
              </div>
