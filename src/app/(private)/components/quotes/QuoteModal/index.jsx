...
export default function QuoteModal({
    ...
    const initForm = async () => {
      if (quote && isOpen) {
        ...
        try {
          ...
          const { data: fullQuote } = await axios.get(
            `/api/quotes/${quote.id}`,
          );

          setFormData({
            customer_id: fullQuote.customer_id,
            date: fullQuote.issued_at
              ? new Date(fullQuote.issued_at).toISOString().split("T")[0]
              : fullQuote.date
                ? new Date(fullQuote.date).toISOString().split("T")[0]
                : "",
            valid_until: fullQuote.valid_until
              ? new Date(fullQuote.valid_until).toISOString().split("T")[0]
              : "",
            ...
            terms:
              fullQuote.terms ||
              "Valable 30 jours. Acompte de 50% à la commande.",
          });
        ...
        const validDate = new Date(today.setMonth(today.getMonth() + 1))
          .toISOString()
          .split("T")[0];
  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      const companyRes = await axios.get("/api/company/get");
      const companyData = companyRes.data.company;

      // 2. Générer le PDF en tant que Blob
      const pdfBlob = await exportQuoteToPDF(res.data, companyData, true);
      ...
      toast.success("Le devis a été envoyé avec succès !", {
        id: "email-send",
      });
  ...
  const handleRequestSignature = async () => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.error || "Erreur lors de l'envoi de la demande",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="fixed inset-0 z-100 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div
        className="bg-white rounded-3xl w-full max-w-5xl h-[90vh] flex flex-col shadow-2xl animate-in zoom-in-95 duration-200"
      ...
      >
        ...
        <form
          ...
          className="flex-1 overflow-y-auto p-4 md:p-10 space-y-6 md:space-y-8 bg-slate-50/50"
        >
          <QuoteClientInfo
            formData={formData}
            customers={customers}
            ...
            onToggleInterventionAddress={() =>
              setShowInterventionAddress(!showInterventionAddress)
            }
          />
