...
const getStatusBadge = (status) => {
    const styles = {
        active: 'background: #dcfce7; color: #166534; padding: 4px 10px; border-radius: 12px; font-size: 11px; font-weight: 600;',
        inactive: 'background: #f1f5f9; color: #64748b; padding: 4px 10px; border-radius: 12px; font-size: 11px; font-weight: 600;',
...
export const exportCustomersToCSV = (customers, filename = 'clients') => {
    const headers = ['Type', 'Nom/Société', 'Contact', 'Email', 'Téléphone', 'Ville', 'Tags', 'Statut'];
    const csvContent = [
        headers.join(','),
        ...customers.map(c => [
            c.type === 'professional' ? 'Pro' : 'Particulier',
            c.type === 'professional' ? c.company_name : `${c.first_name} ${c.last_name}`,
            c.type === 'professional' ? `${c.first_name} ${c.last_name}` : '-',
            c.email || '-',
            c.phone || '-',
            c.city || '-',
            (c.tags || []).join(';'),
            c.status === 'lead' ? 'À relancer' : c.status === 'inactive' ? 'Inactif' : 'Actif'
 ...
 */
export const exportCustomersToPDF = async (customers, filename = 'clients', title = 'Liste des Clients', subtitle = 'Gestion de la Relation Client') => {
