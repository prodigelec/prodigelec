...
export default function CustomerList() {
  ...
  const filteredCustomers = customers.filter((c) => {
    if (filterType === "professional" && c.type !== "professional")
      return false;
  ...
  const handleBulkDelete = async () => {
    ...
    if (protectedCustomers.length > 0) {
      toast.error(
        `Impossible de supprimer ${protectedCustomers.length} client(s) car ils possèdent des devis.`,
      );
      return;
    }
    ...
    try {
      await Promise.all(
        selectedIds.map((id) => axios.delete(`/api/customers/${id}`)),
      );
      ...
      const msg =
        error.response?.data?.error || "Erreur lors de la suppression groupée";
      toast.error(msg);
    } finally {
