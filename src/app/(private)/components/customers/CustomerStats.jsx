...
export default function CustomerStats({ customers = [] }) {
    ...
    const individualsCount = customers.filter(c => c.type === 'individual').length;
    ...
    return (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {stats.map((stat, index) => (
                <div key={index} className={`p-4 rounded-2xl border ${stat.borderColor} bg-white shadow-sm flex items-center gap-4 transition-transform hover:scale-[1.02]`}>
