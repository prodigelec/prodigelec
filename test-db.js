...
async function testDB() {
    ...
    try {
        const { data: customers, error: custError } = await supabase.from('customers').select('count').limit(1);
        if (custError) console.error('Customers Table Error:', custError.message);
        else console.log('Customers Table: OK');

        const { data: quotes, error: quoteError } = await supabase.from('quotes').select('count').limit(1);
        ...
        const { data: joinTest, error: joinError } = await supabase
            ...
            .limit(1);
        if (joinError) console.error('Join Error (Quotes -> Customers):', joinError.message);
