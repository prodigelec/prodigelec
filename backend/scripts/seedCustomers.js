...
async function seed() {
  ...
  try {
    ...
    if (!company) {
      console.error(
        "❌ Erreur: Aucune entreprise trouvée. Veuillez d'abord remplir l'onboarding.",
      );
      process.exit(1);
    }
    ...
    if (Object.keys(typeMap).length === 0) {
      console.error(
        "❌ Erreur: Aucun type de client trouvé. Veuillez lancer le script seedCustomerTypes.js d'abord.",
      );
      process.exit(1);
    }
    ...
    for (const customer of customers) {
      ...
      if (existingCustomer) {
        console.log(
          `⏩ Client déjà existant: ${data.firstName || ""} ${data.lastName || data.companyName}`,
        );
        continue;
      }
