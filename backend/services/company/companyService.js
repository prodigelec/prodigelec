...
const createCompany = async (companyData) => {
  ...
  const company = await prisma.company.create({
    data: {
      ...
      decennaleValidity: decennale_validity
        ? new Date(decennale_validity)
        : null,
    },
  });
...
const updateCompany = async (id, companyData) => {
  ...
  if (companyData.vat_number !== undefined)
    data.vatNumber = companyData.vat_number;
  ...
  if (companyData.legal_form !== undefined)
    data.legalForm = companyData.legal_form;
  ...
  if (companyData.rcs_number !== undefined)
    data.rcsNumber = companyData.rcs_number;
  if (companyData.decennale_number !== undefined)
    data.decennaleNumber = companyData.decennale_number;
  if (companyData.decennale_company !== undefined)
    data.decennaleCompany = companyData.decennale_company;
  if (companyData.decennale_validity !== undefined) {
    data.decennaleValidity = companyData.decennale_validity
      ? new Date(companyData.decennale_validity)
      : null;
  }
