  ...
  try {
    ...
    if (error.code === "P2002") {
      const businessError = new Error(
        "Un client avec ce nom et ce prénom existe déjà pour cette société.",
      );
...
const updateCustomer = async (id, companyId, customerData) => {
  ...
  try {
    ...
    if (customerData.zip_code !== undefined)
      data.zipCode = customerData.zip_code;
    if (customerData.siret !== undefined) data.siret = customerData.siret;
    if (customerData.vat_number !== undefined)
      data.vatNumber = customerData.vat_number;
    if (customerData.delivery_address !== undefined)
      data.deliveryAddress = customerData.delivery_address;
    if (customerData.delivery_city !== undefined)
      data.deliveryCity = customerData.delivery_city;
    if (customerData.delivery_zip_code !== undefined)
      data.deliveryZipCode = customerData.delivery_zip_code;
    if (customerData.payment_terms !== undefined)
      data.paymentTerms = customerData.payment_terms;
...
const deleteCustomer = async (id, companyId) => {
  ...
  if (quotesCount > 0) {
    const businessError = new Error(
      "Impossible de supprimer ce client car il possède des devis ou factures associés.",
    );
