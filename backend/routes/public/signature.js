...
router.post('/signature/create', verifyPublicToken, async (req, res, next) => {
    try {
        ...
        if (tokenInfo.documentId !== documentId || tokenInfo.documentType !== documentType) {
        ...
        console.error('Erreur lors de la création de la signature publique:', error);
        next(error);
    }
});

// Vérifier la validité d'une signature publique
router.get('/signature/verify/:signatureId', verifyPublicToken, async (req, res, next) => {
    try {
...
router.get('/signature/:signatureId', verifyPublicToken, async (req, res, next) => {
    try {
        const { signatureId } = req.params;
        const tokenInfo = req.tokenInfo;
        ...
        console.error('Erreur lors de la récupération des détails de la signature:', error);
        next(error);
    }
});
