...
const getSitesByCustomer = async (req, res, next) => {
    try {
        const sites = await siteService.getSitesByCustomerId(req.params.customerId, req.user.company_id);
        res.json(sites);
    } catch (error) {
        next(error);
    }
...
const getSiteById = async (req, res, next) => {
    try {
        const site = await siteService.getSiteById(req.params.id, req.user.company_id);
        res.json(site);
    } catch (error) {
        next(error);
    }
...
const updateSite = async (req, res, next) => {
    try {
        const site = await siteService.updateSite(req.params.id, req.user.company_id, req.body);
        res.json(site);
    } catch (error) {
        next(error);
    }
