...
class SignatureController {
  ...
  async generateSignatureCertificate(req, res, next) {
    try {
      ...
      const signature = await signatureService.getSignature(signatureId);
      const auditReport = signatureService.generateAuditReport(
  ...
  async revokeSignature(req, res, next) {
    try {
      ...
      // Vérifie que la signature appartient à l'entreprise de l'utilisateur
      if (signature.companyId !== companyId) {
        return res.status(403).json({
          success: false,
          error: "Vous n'êtes pas autorisé à révoquer cette signature",
        });
      }
